...
function msg(pojo, update) {
    ...
    return (<div class={(msg_class(hs, update))}>
...
export function form(pojo, $d, fnSubmit, ffid, ffobj, formFlags, content) {
    ...
    var el = (<form class={(class_prefix + (7 /* MASK_STATUS */ & pojo_.state) + (toggle32 && !(32 & pojo_.state) && 'd-none' || ''))}>
  {!bottom && content}
  {body_out}
  {bottom && content}
  {msg(pojo, update)}
...
function field_bool(pojo, fd, fk, root, ffid) {
    var cls = fd.m === 2 ? 'form-group required' : 'form-group', input;
    var el = (<div class={cls}>
  <label class="form-switch">
    <input ref={input} type="checkbox" checked={(!!pojo[fk])} onChange={function (e) { return $change(e, fk, pojo, root.update, root.pojo); }}/>

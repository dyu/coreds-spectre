...
function field_switch(pojo: any, fd: any, fk: string, root: FormRoot, idx: number, ffid: any) {
    ...
    if (t !== FieldType.BOOL && 0 === (root.flags & FormFlags.PLACEHOLDER)) {
        label = html/**/`<label class="form-label">${fd.$n + (fd.m === 2 && ' *' || '')}</label>`/**/
    }
...
`/**/
    let input = el.firstElementChild.firstElementChild
...
`/**/
    let select = el.lastElementChild
...
function help_text(str) {
    return html/**/`<p class="form-input-hint">${str}</p>`/**/
}
...
<div class=${() => field_class(pojo, fd, fk)}>
  ${label}
  <input type="text" placeholder=${ph} value=${() => fnVal(pojo[fk])} onchange=${e => $change(e, fk, pojo, root.update, root.pojo)} />
  <div class="form-input-hint">${() => !(flag & pojo_.vfbs) ? '' : pojo_[fk]}</div>
  ${hint}
</div>
`/**/
    let input = label ? el.firstElementChild.nextElementSibling : el.firstElementChild
...
<div class=${() => field_class(pojo, fd, fk)}>
  ${label}
  <textarea placeholder=${ph} value=${() => pojo[fk]} onchange=${e => $change(e, fk, pojo, root.update, root.pojo)}>
  </textarea>
  <div class="form-input-hint">${() => !(flag & pojo_.vfbs) ? '' : pojo_[fk]}</div>
  ${hint}
</div>
`/**/
    let ta = label ? el.firstElementChild.nextElementSibling : el.firstElementChild
...
<div class=${field_class(pojo, fd, fk)}>
  ${label}
  <input type=${typ} placeholder=${ph} value=${() => pojo[fk]} onchange=${e => $change(e, fk, pojo, root.update, root.pojo)} />
  <div class="form-input-hint">${() => !(flag & pojo_.vfbs) ? '' : pojo_[fk]}</div>
  ${hint}
</div>
`/**/
